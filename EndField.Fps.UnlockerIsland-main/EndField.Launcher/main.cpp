...
static std::wstring GetIL2CPPDumperDllPath()
{
    std::wstring base = GetModuleDirectory();
    std::wstring local = base + L"\\Cheat.dll";
    if (FileExistsW(local))
        return local;
    return L"";
}
...
static bool LaunchGameWithOptionalInjection(const LauncherConfig& config, std::wstring* error)
{
    ...
    if (config.inject_enabled)
    {
        ...
        {
            // Inject IL2CPP Dumper DLL
            std::wstring il2cpp_dll_path = GetIL2CPPDumperDllPath();
            if (!il2cpp_dll_path.empty())
            {
                if (!InjectDllLoadLibrary(pi.hProcess, il2cpp_dll_path, error))
                {
                    TerminateProcess(pi.hProcess, 1);
                    result = false;
                }
            ...
            // Resume the thread only if all injections succeeded
            if (result)
            {
